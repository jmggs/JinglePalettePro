...
void SettingsDialog::setupUi()
{
    ...
    // ── Remote tab ──────────────────────────────────────────────────────
    {
        ...
        m_spRemotePort->setValue(8000);
        m_ckRemoteOn = new QCheckBox(tr("Enable remote control (HTTP)"));
        f->addRow(tr("Remote port:"), m_spRemotePort);
        f->addRow("", m_ckRemoteOn);
        tabs->addTab(w, tr("Remote"));
    }
...
{
    // Remote
    if (m_spRemotePort) m_spRemotePort->setValue(m_set->remotePort > 0 ? m_set->remotePort : 8000);
    if (m_ckRemoteOn) m_ckRemoteOn->setChecked(m_set->remoteOn);
    // Display
...
void SettingsDialog::accept()
{
    ...
    // Remote
    if (m_spRemotePort) m_set->remotePort = m_spRemotePort->value();
    if (m_ckRemoteOn)   m_set->remoteOn   = m_ckRemoteOn->isChecked();
    QDialog::accept();
}
