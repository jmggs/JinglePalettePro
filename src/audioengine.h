...
private slots:
    ...
    void onVuTimer();
    void onAutoMixTimer();
    void onLevelDecoderBuffer();
    void onLevelDecoderFinished();
    void onLevelDecoderError(QAudioDecoder::Error err);
...
private:
    ...
    int            m_vuTick        = 0;   // contador para simular dinamica

    // PCM level decoder + queue
    QAudioDecoder              *m_levelDecoder     = nullptr;
    int                         m_levelDecIdx      = -1;
    int                         m_levelDecFrameCnt = 0;
    float                       m_levelDecSumL     = 0.f;
    float                       m_levelDecSumR     = 0.f;
    bool                        m_levelDecBusy     = false;
    QQueue<QPair<int,QString>>  m_decodeQueue;
